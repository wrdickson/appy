<template>
  ...
  <el-table
    ...
    style="width: 100%"
  >
    <el-table-column prop="paymentTypeName" label="Type"/>
...
<script setup>
  ...
  const postPayment = () => {
    ...
    showAddPayment.value = ref(false)
    //selectedPaymentType.value = ref(null)
    paymentAmount.value = null
    payments.value = []
  }
  ...
  const paymentsEqualSaleAmount = computed( () => {
    if( props.total == paymentsTotal.value && payments.value.length > 0 ){
      return true
    } else {
      return false
    }
  ...
  const paymentsTotal = computed( () => {
    console.log('pT shit', payments.value)
    let pTotal = 0
